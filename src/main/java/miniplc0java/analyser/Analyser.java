...
//                    initialized = true;
//                    instructions.add(new Instruction(Operation.LIT, (Integer) Objects.requireNonNull(nextIf(TokenType.Uint)).getValue()));
//                }
...
//            addSymbol(name, false, false, /* 当前位置 */ null);
    ...
    private void analyseStatementSequence() throws CompileError {
        ...
        while (true) {
            ...
            else if (peeked.getTokenType().equals(TokenType.Print)) {
    ...
    private void analyseAssignmentStatement() throws CompileError {
        ...
        // 标识符是什么？
        String name = identToken.getValueString();
    ...
    private void analyseItem() throws CompileError {
        ...
        while (true) {
            ...
            // 运算符
            if (!(op.getTokenType().equals(TokenType.Mult) || op.getTokenType().equals(TokenType.Div))){
                break;
            }
            next();
    ...
    private void analyseFactor() throws CompileError {
        ...
        if (check(TokenType.Ident)) {
            ...
            if (symbol == null) {
                // 没有这个标识符
                throw new AnalyzeError(ErrorCode.NotDeclared, identToken.getStartPos()/* 当前位置 */ );
            } else if (!symbol.isInitialized) {
                // 标识符没初始化
                throw new AnalyzeError(ErrorCode.NotInitialized, /* 当前位置 */ identToken.getStartPos());
            }
            var offset = getOffset(name, identToken.getStartPos());
            instructions.add(new Instruction(Operation.LOD, offset));
        } else if (check(TokenType.Uint)) {
            // 是整数
            // 加载整数值
            Token intToken = next();
            int value = (Integer)intToken.getValue();
            instructions.add(new Instruction(Operation.LIT, value));
